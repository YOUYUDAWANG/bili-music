...
extension UIView {
	...
	func __animateUsingSwiftUI(duration: TimeInterval, changes: @escaping () -> Void, completion: (() -> Void)? = nil) {
		__animateUsingSwiftUI(duration: duration, delay: 0.0, changes: changes, completion: completion)
	}

	@objc(_ln_animateUsingSwiftUIWithDuration:delay:animations:completion:)
	static private
	func __animateUsingSwiftUI(duration: TimeInterval, delay: TimeInterval, changes: @escaping () -> Void, completion: (() -> Void)? = nil) {
		if #available(iOS 18.0, *) {
			UIView.animate(.spring(.snappy(duration: duration)).delay(delay), changes: changes, completion: completion)
		} else {
			UIView.animate(withDuration: duration, delay: delay, usingSpringWithDamping: 500, initialSpringVelocity: 0.0, animations: changes) { _ in
				completion?()
			}
		}
	}

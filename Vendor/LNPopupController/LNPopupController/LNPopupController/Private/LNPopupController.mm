...
// Keep the full-player dock/undock motion deliberate enough to read as one
// continuous surface on ProMotion displays instead of a fast modal jump.
static const NSTimeInterval LNPopupContentTransitionDuration = 0.62;
...
- (void)_setInteractiveContentRasterizationEnabled:(BOOL)enabled
{
	CALayer* contentLayer = _currentContentController.view.layer;
	contentLayer.rasterizationScale = enabled ? _containerController.view.window.screen.scale : 1.0;
	contentLayer.shouldRasterize = enabled;
}
...
- (void)_transitionToState:(LNPopupPresentationState)state notifyDelegate:(BOOL)notifyDelegate animated:(BOOL)animated triggeredByGesture:(BOOL)triggeredByGesture velocity:(CGPoint)velocity allowPopupBarAlphaModification:(BOOL)allowBarAlpha allowFeedbackGeneration:(BOOL)allowFeedbackGeneration forceFeedbackGenerationAtStart:(BOOL)forceFeedbackAtStart completion:(void(^)(void))completion
{
	...
	void (^updatePopupBarAlpha)(void) = ^ {
		...
		{
			self.popupBar.layoutContainer.alpha = 1.0;
		}
	};
...
//	[self _clearRunningPopupAnimators];
	...
	CGFloat animationDuration = LNPopupContentTransitionDuration;
#if DEBUG
...
- (void)_popupBarPresentationByUserPanGestureHandler_changed:(UIPanGestureRecognizer*)pgr
{
	...
	if(_dismissGestureStarted == NO && (resolvedStyle == LNPopupInteractionStyleDrag || resolvedStyle == LNPopupInteractionStyleScroll || _popupControllerInternalState > LNPopupPresentationStateBarPresented))
	{
		...
		// During a slow interactive close the content only translates vertically.
		// Cache the complete SwiftUI player as one screen-scale layer so gradients,
		// artwork shadows and hosted side pages are not recomposited every frame.
		[self _setInteractiveContentRasterizationEnabled:YES];
	...
	if(_dismissGestureStarted == YES)
	{
		...
		if(resolvedStyle == LNPopupInteractionStyleSnap && realTargetCenterY / self.popupBar.superview.bounds.size.height > _containerController.popupSnapPercent)
		{
			...
			[self closePopupAnimated:YES triggeredByGesture:YES velocity:[pgr velocityInView:pgr.view] allowFeedbackGeneration:YES forceFeedbackGenerationAtStart:YES completion:^ {
				[self _setInteractiveContentRasterizationEnabled:NO];
...
//			[self _clearRunningPopupAnimators];
			...
			_runningBarAnimation = [[UIViewPropertyAnimator alloc] initWithDuration:animationDuration dampingRatio:500 animations:^{
				...
				CGFloat currentBarAlpha = self.popupBar.layoutContainer.alpha;
				if(!LNPopupEnvironmentHasGlass() && animated)
				{
					[UIView animateWithDuration:animationDuration delay:0.0 usingSpringWithDamping:500 initialSpringVelocity:0 options:UIViewAnimationOptionAllowAnimatedContent animations:^{
						...
						{
							self.popupBar.layoutContainer.alpha = 0.0;
						}
						_containerController._ln_bottomBarExtension_nocreate.alpha = 0.0;
					} completion:^(BOOL finished) {
						self.popupBar.layoutContainer.alpha = currentBarAlpha;
					}];
				}

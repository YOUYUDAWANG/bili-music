...
| `PlayerGesturePolicyTests.swift` | `PlayerGesturePolicy` 全量（mini 上滑开启、封面 matchedGeometry 归属、区域化下滑关闭、列表横滑抑制点按）+ `ProgressScrubMath`（progress clamp、`shouldBeginScrub` 水平意图判定） |
...
| `SearchModelsTests.swift` | `SearchCacheKey` 归一化、`SearchResultMode`、`SearchResultSections` 三段拆分、`MusicFilter.isSearchResult` 过滤、SearchStore 缓存恢复与模式切换 |
| `SearchStoreTests.swift` | `loadLocalContent`/`refreshLocalContent` 投影与 contentRevision 增量语义、空查询不清结果、分页部分失败保留成功页、缓存 `entry(for:)` 分 P 精确匹配 |

### UI 测试（`BiliMusicUITests/`）

| 文件 | 覆盖内容 |
|------|---------|
| `PlayerChromeUITests.swift` | mini player 上滑打开全屏播放器、顶部 chrome 下滑关闭、封面/标题在多次开合中的存活、安全区约束 |
...
- **UI 测试 fixture**：`BILIMUSIC_UITEST_FIXTURE=1` 环境变量（`BiliMusic/Support/UITestFixtures.swift`）。App 启动时注入固定曲目队列（`UITestFixtures.homeTracks`），`PlayerEngine.installUITestFixture(tracks:startAt:)` 不建真实流。UI 测试通过 `app.launchEnvironment` 设置。
- **调试环境变量**：`AUTOPLAY_BV`（模拟器自动播放）、`AUTOPLAY_TEST_NEXT`（测切歌）。

## 关键约束

- 只做单元测试 + 编译验证；交互体验最终在真机 iPhone 上验证（AltStore 签名）。
- 不发真实网络请求；LyricsClient/BiliClient 无单元测试（依赖网络）。
- 避免「读生产源码断言精确缩进子串」的脆弱写法；行为已有策略函数（`PlayerGesturePolicy`、`RecommendationPanelRefreshPolicy` 等）时优先断言策略函数行为。

...
- [x] 播放器保持 16:9；mini player 继续使用 LNPopup 原生开合，首页封面通过局部 matched geometry 动画层从被点位置原位展开并反向缩回，缩回期间瀑布流仍可操作；封面接近屏幕边缘，字幕沿封面左缘排布，背景收敛为封面双色光场。
...
- **UI**: Apple Music 只作为系统交互参考；原生 Tab Bar 与 mini player 保持 Liquid Glass，首页和播放器使用横版影像唱片机视觉。首页封面原位放大是用户确认的核心交互，但不能以常驻全部 Tab、自绘底栏、重复播放状态或全屏竞争手势实现。搜索、缓存等工具页保持高效率，播放器必须适合单手触控和反复使用。
...
| 首页封面从原位置展开播放器 | 用户明确提出并确认该连续性；首页局部 matched geometry 负责放大和反向缩回，关闭第一帧释放命中测试，使瀑布流在动画期间可滚动，完成后恢复标准 mini player | Implemented and simulator-regression-tested |

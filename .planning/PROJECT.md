...
- [x] Windows 高精度逐字歌词主机已作为显式来源接入：认证异步任务、GPU 串行、结果缓存与 App 二次质量门禁已用「You＆合図」真实闭环验证。
- [x] LDDC 已作为独立 GPL 私有逐字歌词服务接入 Swift 解析器，并部署到 Mac mini Tailscale 常驻主机：双重门禁、直连回退、认证、LaunchAgent 自启和真实远程请求已验证；20 首 PoC 精确逐字覆盖由 2 首提升到 13 首。
...
- [x] 首页先展示收藏夹封面快照，再把首页推荐流里的新歌混进同一条瀑布流；点歌不等待推荐。缓存和历史只在资料库为空时兜底。
...
- [ ] LDDC 真机验收：服务器部署、钥匙串注入和签名 App 安装已完成；还需在 App 内确认翻唱同版本、原唱参考和直连回退。

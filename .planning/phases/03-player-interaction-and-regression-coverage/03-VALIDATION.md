...
| PLYR-05 | Dense Apple Music-like layout has no excessive bottom void on supported iPhone sizes | UI + manual UAT | `xcodebuild test ... -only-testing:BiliMusicUITests/PlayerChromeUITests` plus real-device visual check | gap | pending |
...
- [ ] `BiliMusicUITests/PlayerChromeUITests.swift` covers queue-page scroll, recommendation-page scroll, progress scrub not paging/dismissing, page swipe not minimizing, and dense layout identifiers.

...
B 站接口客户端层。所有对 B 站 API 的 HTTP 调用都在此模块内，包括视频信息、音频/视频流、搜索、相关推荐、扫码登录、收藏夹 CRUD、UP 主合集/系列、首页推荐流、用户信息。
...
| `videoStream(bvid:cid:profile:)` | 视频 MP4 流 URL | 否 |
| `search(keyword:page:musicOnly:)` | 搜索视频（WBI 签名） | 否 |
| `related(bvid:)` | 相关推荐视频 | 否 |
| `subtitles(bvid:cid:)` | 字幕轨列表 | 否 |
| `subtitleFile(_:)` | 下载指定字幕轨 | 否 |
...
| `homeFeed(freshIdx:)` | 首页推荐流（WBI 签名） | 否（登录后个性化） |
...
- `lyrics(for:)` —— 为 Track 匹配同步歌词。流程：解析歌名 → LRCLIB 搜索 → 歌名相似 + 时长（差 ≤12s）匹配 → LRC 解析为 `[PlayerEngine.LyricLine]`。
